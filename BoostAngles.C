...
void BoostAngles(Int_t nSigma=3
  ...
  Double_t nBGSR = fBG->Integral(massMin,massMax);

  fInmass->Close();
  fIn->cd();
  
  lepP = 0; lepN = 0;
  Double_t Nch, pT, costh_HX, phi_HX, costh_CS, phi_CS, onia_mass, onia_rap, wS;
...
////////////////////////////
  ...
  if(onia_mass > massMin && onia_mass < massMax) wS = 1;
  else wS = 1 - (nBGSB+nBGSR)/nBGSB;
